...
$VERSIONS[basename(__FILE__)] = "3.2 PRO";
...
if(count($data1)) {
    while(list(,$d) = each($data1)) {
        ...
        $TMP['%[CONTENT]%'] .= '<tr><td style="padding:4px" align=center'.$col.'>'.date($CONFIG['date_format'],strtotime($d['regdat'])+(3600*$CONFIG['timezone'])).'</td><td align=center'.$col.'><img src="sp.php?u='.$d['userid'].'" width="40"></td><td'.$col.' style="padding:1 10px"><b><a href="'.$href.'" '.$blank.$onClick.'>'.htmlspecialcharsISO(stripslashes($d['anzeige'])).'</a></b></td><td style="padding:0 10px" align=right'.$col.'>'.($d['kaesten']*$d['blocksize_y']*$d['blocksize_x']).'</td></tr>';
...
$TMP['%[SEARCHFORM]%'] = '
    ...
    <input type="text" name="f" style="width:200px" value="'.htmlspecialcharsISO(stripslashes($_GET['f'])).'">
