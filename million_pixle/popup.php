...
var is_ff = (agt.indexOf("firefox") != -1);
    if(is_op) {
        ...
        is_nn = false;
        is_ff = false;
        var basewidth = eval(<?php print $popupwidth; ?>+4);
        var baseheight = eval(<?php print $popupheight; ?>);
        var baseiframe = eval(<?php print $popupheight; ?>);
        var basepatch = 0;
    }
    if(is_ie) {
        var basewidth = eval(<?php print $popupwidth; ?>+4);
        var baseheight = eval(<?php print $popupheight; ?>+4);
        var baseiframe = "100%";
        var basepatch = 2;
    }
    if(is_nn) {
        var basewidth = eval(<?php print $popupwidth; ?>);
        var baseheight = eval(<?php print $popupheight; ?>+20);
        var baseiframe = eval(<?php print $popupheight; ?>);
        var basepatch = 0;
    }
    if(is_ff) {
...
function minimieren() {
    document.getElementById('popup<?php print $popupID?>').style.display='none';
    var PopupFenster<?php print $popupID?> = window.open("<?php print $popupurl; ?>","PopupFenster<?php print $popupID?>","left=20,top=100,width=<?php print $popupwidth; ?>,height=<?php print $popupheight; ?>,toolbar=no,menubar=no,status=no,scrollbars=<?PHP print $POPUP_NOSCROLLING ? 'no' : 'yes';?>,resizable=no,location=no");
    PopupFenster<?php print $popupID?>.blur();
}
 if (is_op) {
  var PopupFenster<?php print $popupID?> = 0;
 } else {
  var PopupFenster<?php print $popupID?> = window.open("<?php print $popupurl; ?>","PopupFenster<?php print $popupID?>","left=20,top=100,width=<?php print $popupwidth; ?>,height=<?php print $popupheight; ?>,toolbar=no,menubar=no,status=no,scrollbars=<?PHP print $POPUP_NOSCROLLING ? 'no' : 'yes';?>,resizable=no,location=no");
 }
 if (PopupFenster<?php print $popupID?>) {
   PopupFenster<?php print $popupID?>.focus();
 } else {
        try {
