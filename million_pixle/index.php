    ...
    if((int)$_GET['u']) {
        if($ENTRY = DB_Query("SELECT * FROM ".$dbprefix."user t0 LEFT JOIN ".$dbprefix."grids t1 ON(t0.gridid=t1.gridid) WHERE userid='".(int)$_GET['u']."'",'*')) {
            ...
            if($ENTRY['popup'] && empty($ENTRY['url'])) {
                $tmp['%[URL]%']               = htmlspecialcharsISO(stripslashes($ENTRY['url']));
                $tmp['%[URL_LINKED]%']        = '<a href="'.$ENTRY['url'].'" target="_blank">'.$ENTRY['url'].'</a>';
                $tmp['%[EMAIL]%']             = htmlspecialcharsISO(stripslashes($ENTRY['email']));
                ...
                $tmp['%[TITLE]%']             = htmlspecialcharsISO(stripslashes($CONFIG['website_title']));
                $tmp['%[TEXT]%']              = htmlspecialcharsISO(stripslashes($ENTRY['title']));
