...
if($login_logged_in) {
    ...
    $TMP['%[INPUT_URL]%'] .= '<input type="text" name="url" class="savepixel_url" maxlength="1000" value="'.htmlspecialcharsISO(stripslashes($logindata['url'])).'" tabindex="3"';
        if($Fehlerfeld['url']) $TMP['%[INPUT_URL]%'].= ' style="border: 1px solid red"';
        $TMP['%[INPUT_URL]%'].= '">';

    $TMP['%[INPUT_TITLE]%'] = '<input type="text" name="title" class="savepixel_inputs" maxlength="'.$logindata['title_chars'].'" value="'.htmlspecialcharsISO(stripslashes($logindata['title'])).'" tabindex="4"';
        if($Fehlerfeld['title']) $TMP['%[INPUT_TITLE]%'] .= ' style="border: 1px solid red"';
        $TMP['%[INPUT_TITLE]%'] .= '">';

    $TMP['%[INPUT_EMAIL]%'] = '<input type="text" name="email" class="savepixel_inputs" maxlength="90" tabindex="5" value="'.htmlspecialcharsISO(stripslashes($logindata['email'])).'"';
    ...
    $TMP['%[LOGINFORM]%'] = $no_loginform ? '' : '
                            <table  cellspacing="2">
                              ...
                              <tr><td align="right"><font color="#0147AA"><b>'.$_SP[108].'&nbsp;</font></td>
                                  <td><input type="text" name="kennung" size="30" tabindex="1" style="color:#000000" value="'.htmlspecialcharsISO(strip_tags(stripslashes($_POST['kennung']))).'" maxlength="50"></td>
                              <tr><td colspan="3"></td></tr>
